...
let barHeight = 180; // height of each bar
let barMax = 760; // maximum width of each bar <-- this changes over time
...
function draw() {
  ...
  let h = map(hour(), 0, 23, 0, barMax); // Map the function hour() to values from 0 - barMax
  let m = map(minute(), 0, 60, 0, barMax); // Map the function minute() to values from 0 - barMax
  let s = map(second(), 0, 60, 0, barMax); // Map the function second() to values from 0 - barMax
